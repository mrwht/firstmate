...
`tests/fm-watcher-lock.test.sh` covers verified-successor attach, recovery publication before stale-lock removal, the typed self-eviction failure, bounded and successor-linked lifecycle rows, a SIGSTOP counterfactual that distinguishes a live PID from a stale beacon before classifying termination, and the `state/.watch-arm-track.lock` dedup guard that makes a redundant concurrent plain-mode arm call exit promptly instead of leaking a second indefinite follower.
