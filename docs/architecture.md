...
Before any of that, it compares the worktree's live checked-out branch against the crew's own branch recorded in `state/<id>.meta` at spawn (or promotion): a pooled worktree silently reassigned to a different task reports unknown rather than a stranger's run or pane state, while a meta written before that field existed falls through unchanged.
...
For whole-fleet read-only review, `bin/fm-fleet-snapshot.sh --json` emits schema `fm-fleet-snapshot.v2` from the backlog, task metadata, current crew state, endpoint probes, PR/report pointers, and scout reports.
...
[`watcher-continuity.md`](watcher-continuity.md#arm-layer-cycle-contract) owns the arm layer's successor, terminal-delivery, re-arm recovery, indefinite-follow single-claim, and typed clean-close failure contract.
...
The guard covers the main primary, exempts child crewmate/scout worktrees, is loop-safe per harness, and is documented in [turnend-guard.md](turnend-guard.md).
...
Before that reset, it also refuses to recycle a pooled worktree that another live task's `state/<id>.meta` still claims: it scans `$FM_ROOT`'s own worktree list for every home sharing that repo's object store, not the pooled worktree's own repo, since a candidate pooled worktree is normally a checkout of an unrelated managed project rather than of firstmate itself; an unlisted, unreadable, or unverifiable claim refuses the spawn rather than treating the worktree as unclaimed.
Its header owns the exact refusal mechanics, while `tests/fm-spawn-pool-base-freshen.test.sh` owns the portable regression coverage.

The firstmate repo has one extra exposure because it can dispatch crewmates to work on itself.
Its operating checkout (`FM_ROOT`) and the disposable crewmate worktrees are all linked git worktrees of the same repository, so the valid discriminator is branch state, not whether the checkout is linked.
The primary checkout is healthy on its default branch, and linked worktrees are healthy at detached HEAD.
...
`/updatefirstmate` fast-forwards the running firstmate repo's default branch from `origin`, then re-reads updated instructions, without touching project clones or secondmate homes.
...
A dead secondmate agent endpoint is no longer auto-relaunched at session start; recovering it through the same secondmate spawn path is a manual, digest-driven action owned by the [`secondmate-provisioning` skill](../.agents/skills/secondmate-provisioning/SKILL.md#recovery).
