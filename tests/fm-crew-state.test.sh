...
# detail: a healthy idle crew that just closed a keyed decision falls through to
# the idle default (unknown/none), not `unknown` with the resolution note as its
# `doing`. Regression for the bearings render bug where a resolved-then-idle
# task showed state=unknown with resolution prose. The one-owner keyed fold in
# fm-classify-lib.sh is untouched; this only stops the deriver from reading a
# non-state event as state.
test_no_run_idle_resolved_event_not_state() {
  reset_fakes
  local d; d=$(new_case resolved-idle)
  mkdir -p "$d/wt"
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/mate.meta" "window=fm:fm-mate" "worktree=$d/wt" "kind=ship" "harness=claude"
  ...
  assert_contains "$out" "state: unknown" "resolved-then-idle crew is not a spurious run-state"
