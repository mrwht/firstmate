...
test_workspace_label_resolves_to_firstmate() {
  local home
  ...
  [ "$out" = "firstmate" ] || fail "a home should resolve to label 'firstmate', got '$out'"
  pass "fm_backend_herdr_workspace_label: resolves to 'firstmate'"
}
...
test_workspace_find_matches_only_this_homes_own_label() {
  ...
  # A workspace list carrying this home's own "firstmate" space alongside
  # unrelated human-created spaces - find must pick the one matching THIS
  # home's own label, never a differently labeled workspace.
  printf '{"result":{"workspaces":[{"workspace_id":"w1","label":"human-space"},{"workspace_id":"w2","label":"firstmate"},{"workspace_id":"w3","label":"other-app"}]}}\n' > "$resp/1.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_workspace_find fmtest' "$ROOT" )
  [ "$out" = "w2" ] || fail "workspace_find should have matched this home's own label (firstmate -> w2), got '$out'"
...
test_list_live_scoped_to_this_homes_workspace_only() {
  ...
  # 1: workspace_find's `workspace list` - this home's own space is w2
  printf '{"result":{"workspaces":[{"workspace_id":"w1","label":"human-space"},{"workspace_id":"w2","label":"firstmate"}]}}\n' > "$resp/1.out"
  # 2: tab list --workspace w2 (this home's own tabs only)
  printf '{"result":{"tabs":[{"tab_id":"w2:t1","label":"fm-sometask"}]}}\n' > "$resp/2.out"
  # 3: pane_for_tab's `pane list --workspace w2`
  printf '{"result":{"panes":[{"pane_id":"w2:p1","tab_id":"w2:t1"}]}}\n' > "$resp/3.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_list_live fmtest' "$ROOT" )
  [ "$out" = $'fmtest:w2:p1\tfm-sometask' ] || fail "list_live should report only this home's own tab, got '$out'"
  ...
  assert_not_contains "$(cat "$log")" $'\x1f''tab'$'\x1f''list'$'\x1f''--workspace'$'\x1f''w1' \
    "list_live must never query a differently labeled workspace"
  pass "fm_backend_herdr_list_live: scoped to this home's own workspace, never another"
}
