...
#
# Firstmate is a treehouse-pooled git repo of itself: crewmate worktrees are
# linked `git worktree`s of the same repo, while the PRIMARY checkout (the repo
# root firstmate operates from) is a normal checkout on a real branch -
# normally the default branch, main. The "worktree tangle" failure mode is a
# crewmate spawned to work on firstmate ITSELF branching and committing in the
# primary checkout instead of its own disposable worktree, stranding the
# primary on a feature branch (e.g. fm/readme-restructure-d3).
#
...
# which is how every linked worktree legitimately sits on the default branch.
# Detached HEAD on the default is fine; a feature branch in a primary checkout
# is the alarm.
...
# echo nothing and return 1. Detached HEAD is how linked worktrees legitimately
# sit, so they never trip this; only a feature branch checked out in a primary
# checkout does.
